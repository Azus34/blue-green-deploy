...
# Descripción: Despliega o actualiza la aplicación en el entorno GREEN
# Uso: ./scripts/deploy-green.sh <IMAGE_URI>
# Ejemplo: ./scripts/deploy-green.sh ghcr.io/usuario/app:latest
################################################################################
...
{
    echo "======================================================================"
    echo "Despliegue Blue-Green: GREEN Environment"
    echo "Fecha: $(date '+%Y-%m-%d %H:%M:%S')"
    echo "Image: $IMAGE_URI"
    echo "======================================================================"
} | tee -a "$LOG_FILE"

log_info "Iniciando despliegue en entorno GREEN..."
...
# Detener y remover contenedor GREEN anterior si existe
log_info "Deteniendo contenedor GREEN anterior (si existe)..."
...
# Crear y ejecutar el nuevo contenedor GREEN
log_info "Creando contenedor GREEN en puerto $PORT..."
...
if docker run \
    ...
    log_success "Contenedor GREEN creado exitosamente"
else
    log_error "Fallo al crear el contenedor GREEN"
    exit 1
fi
...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    ...
    if [ "$HEALTH_STATUS" = "healthy" ]; then
        log_success "Contenedor GREEN está healthy"
        break
    fi
    
    if [ "$HEALTH_STATUS" = "unhealthy" ]; then
        log_error "Contenedor GREEN reporta estado unhealthy"
...
log_success "Despliegue en entorno GREEN completado exitosamente"
echo ""
